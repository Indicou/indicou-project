          ...
          <div class="col-md-12 pt-3 px-3 mb-4">
           <!-- <ul class="breadcrumb bg-light border-muted border-bottom">
            ...
            <h2 class="text-muted pl-4 py-2 mx-4" style="background:linear-gradient(89deg, rgb(239 239 239 / 35%) 0%, rgb(248, 249, 250) 100%);font-weight: 400;margin-left: 4px;border: 1px solid rgb(242 242 242);;border-radius: 6px;">
              <b><i class="fas fa-1x fa-puzzle-piece pr-2" style="color: #91f89a;-webkit-text-stroke: 1px rgb(57, 57, 57);"></i>Editar Anúncio de Indicação</b>
            </h2>
          </div>
        </div>
        <div class="row">
              ...
              <div class="col-md-12 d-flex justify-content-center pb-5 bg-light">
                <button class="logbtn text-center btn-lg w-25 my-3" :disabled="loading">
                    <template v-if="loading">
                      Atualizando Anúncio... <i class="fas fa-spinner fa-spin"></i>
                    </template>
                    <template v-else>
                      Salvar e Atualizar Anúncio
                    </template>
                </button>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    getUser () {
      ...
      db.collection('business').doc(this.idbusiness).collection('Cards').doc(this.idcard).get()
        .then(doc => {
          this.categoria = doc.data().filtro
          this.tiponegocio = doc.data().tiponegocio
          this.titulo = doc.data().titulo
          this.sobre = doc.data().sobre
          this.termos = doc.data().termo
          this.img = doc.data().imagem
          this.regiao = doc.data().regiao
          this.todos = doc.data().locais
          this.tipo.push(doc.data().tipo)
          this.guardcomissao = doc.data().comissao
          this.formpersonalizado = doc.data().forms
          this.fisico = doc.data().fisico
          this.dataCriacao = doc.data().dataCriacao
        })
    }
  }
}
